...
namespace Usbacc.UI.View
{
    ...
    /// </summary>
    public partial class DeviceAccountTableView
    {
        public DeviceAccountTableView()
        {
            InitializeComponent();
        ...
        {
            var dialog = new DeviceAccountAddView { Owner = Window.GetWindow(this) };
            dialog.Closed += (s, e) => dialog.Owner.Focus();
            dialog.ShowDialog();
        }

        private void EditAction(DeviceAccount deviceAccount)
        {
            var dialog = new DeviceAccountAddView(deviceAccount) {Owner = Window.GetWindow(this)};
            dialog.Closed += (s, e) => dialog.Owner.Focus();
            dialog.ShowDialog();
        }
        ...
        private void MainDataGrid_OnMouseDoubleClick(object sender, MouseButtonEventArgs e)
        {
            ...
            if (element != null && element.Parent is DataGridCell)
            {
                ...
                if (datagrid != null)
                {
                    var item = ViewModel.SelectedItem as DeviceAccount;
                    if (item != null)
                        EditAction(item);
                }
            }
